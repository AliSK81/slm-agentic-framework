"""Load frozen curated task-id lists from configs/reporting/."""
...
_DEFAULT_HARD_IDS_PATH = _REPORTING_CONFIGS_DIR / "humaneval_hard_ids.txt"
...
def resolve_ids_path(ids_file: str | None) -> Path:
    """Resolve ``ids_file`` relative to ``configs/reporting/``."""
    if not ids_file:
        return _DEFAULT_HARD_IDS_PATH
    path = Path(ids_file)
    if path.is_absolute():
        return path
    return _REPORTING_CONFIGS_DIR / path
